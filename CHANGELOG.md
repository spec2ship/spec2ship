...
### Fixed
- **BUG-026 (medium)** — the FEAT-012 panel coverage warning ("no technical role on the panel") was a standalone display step and got skipped at runtime (exp50 dogfood, same display-only-step class as BUG-013). The check now sets `PANEL_WARNING` and the warning line is embedded in the always-rendered "Display session start" block of `commands/roundtable.md`.
