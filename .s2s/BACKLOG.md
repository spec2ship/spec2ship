...
| FLOW | Artifact flow/traceability | FLOW-001 |
...
### FLOW-001: Ideas and Artifact Traceability

**Status**: completed | **Created**: 2026-01-19 | **Completed**: 2026-01-19 | **Priority**: High

**Context**: Brainstorm sessions produce ideas that have no structured output. Ideas can come from brainstorm or manual entry. Need traceability from ideas → requirements → backlog → plans.

**Analysis Session**: Conversation 2026-01-19 (artifact flow analysis)

**Decisions Made**:
1. **ideas.md**: New output file for `/s2s:brainstorm` and manual idea entry
2. **No new flags**: LLM detects sources via context, IDs, or recency
3. **Traceability links**: Maintain bidirectional references between artifacts
4. **Status sync**: Manual, but LLM can check coherence on-demand
5. **BACKLOG.md uppercase**: Preserved (important/foundational file convention)

**Structure**:
```
.s2s/
├── CONTEXT.md           # Project context (UPPER - foundational)
├── BACKLOG.md           # Work items (UPPER - foundational)
├── config.yaml          # Configuration
├── ideas.md             # Ideas (lower - generated by brainstorm)
├── requirements.md      # Requirements (lower - generated by specs)
├── architecture.md      # Architecture (lower - generated by design)
├── decisions/           # ADRs
...
**Artifact Flow**:
```
brainstorm/manual → ideas.md (IDEA-*)
                         ↓ promote
specs ←──────────── BACKLOG.md (FEAT-*) ←── can also start from REQ/IDEA
    ↓
requirements.md (REQ-*, NFR-*)
    ↓
design → architecture.md (COMP-*, INT-*) + decisions/ (ARCH-*)
    ↓
plan → plans/ (implements FEAT-*, references REQ-*, ARCH-*)
```

**Entry Points by Command**:
| Command | Primary Sources | Smart Detection |
|---------|-----------------|-----------------|
| `/s2s:specs` | CONTEXT, ideas.md, BACKLOG | Recent brainstorm → propose; ID passed → lookup |
| `/s2s:design` | requirements.md (warn if missing) | Standard flow |
| `/s2s:plan` | BACKLOG, requirements, architecture | Recent BACKLOG items; ID passed → lookup |

**Plan Entry Points** (smart detection without flags):
1. `plan FEAT-001` → reads BACKLOG item
2. `plan REQ-003` → reads requirement
3. `plan COMP-002` → reads component
4. `plan IDEA-005` → **WARNING** + suggest specs first, explicit confirm to proceed
5. `plan "user auth"` → search BACKLOG → requirements → architecture → ideas
6. Nothing specified → show BACKLOG items with status: planned

**Tasks**:
1. [x] Create `templates/project/ideas.md` template
2. [x] Update `templates/project/BACKLOG.md` with traceability fields
3. [x] Update `/s2s:brainstorm` to generate ideas.md output
4. [x] Add smart source detection to `/s2s:specs` (no new flags)
5. [x] Add smart source detection to `/s2s:plan` with IDEA warning
6. [x] Update `templates/plan.md` with source traceability
7. [x] Create `skills/s2s-guide/references/workflow-guide.md` documentation
8. [x] Update `/s2s:init` to create ideas.md
9. [x] Update skills/s2s-guide/ with artifact flow reference

**Acceptance Criteria**:
- [x] brainstorm generates ideas.md with IDEA-* entries (Step 3.6 added)
- [x] specs proposes recent sources without requiring flags (Smart Source Detection section)
- [x] plan warns when starting from unvalidated IDEA (IDEA-* pattern with AskUserQuestion)
- [x] Traceability links work bidirectionally (templates updated, workflow-guide documented)
- [x] LLM can verify coherence on-demand (mentioned in workflow-guide)

**Related**: PLAN-001, RT-005
...
|----|-------------|-----------|-------|
| FLOW-001 | Ideas and Artifact Traceability | 2026-01-19 | ideas.md, smart source detection, IDEA warning, workflow-guide in s2s-guide/references/ |
