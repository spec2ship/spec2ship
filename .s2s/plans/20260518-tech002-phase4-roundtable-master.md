...
2. **Option A/B/C decision executed** — explicit choice (recommendation: **Option B, command-side parsing**, see §3) implemented in roundtable.md so debate `debate_role` + six-hats `hat_role` overrides come from a deterministic source instead of LLM interpretation at Step 2.2c.
...
4. **commands/roundtable.md drift reconciliation** — lines 170-179 keyword auto-detect table and lines 194-199 inline phase enumeration (with phase-name drift versus strategy docs: `consensus-driven` and `six-hats`) are reconciled per the chosen Option.
...
- **Agent prompt redesign**. `agents/roundtable/facilitator.md` continues to be invoked by roundtable.md; Phase 4 may sharpen the strategy-doc pointers (lines 518, 579, 607) to `#strategy-hooks` anchors but does not rewrite the agent.
...
3. **Determinism delivered without new file types**. Strategy docs remain human-facing; structure lives in regex fixtures inside roundtable.md (or a single `references/strategy-hook-anchors.md` table). Drift surface is single-fixture.
...
Phase 4 delivers in 7 sub-phases over an estimated **~5.5 hours** (drafting + execution + smoke):

- **4.0** audit (~1h): inventory of inline duplication, drift sites, Step 2.X dispatch points in roundtable.md
- **4.1** triple-dup resolution (~1h): D3 hierarchy codified, SKILL.md disclaimer + diagram, profile-schema.md updated
- **4.2** Option B parser (~1h): strategy-hook anchors fixture + parse block in roundtable.md
- **4.3** roundtable.md Phase 2 expansion (~1.5h): replace lines 359-437 stub with explicit Step 2.0 → 2.10 dispatch via `phase-2-core.md`, add `--workflow-type` profile load
- **4.4** drift fix lines 170-179 + 194-199 (~0.5h): collapse keyword auto-detect into a single sentence + Read; remove inline phase enumeration; reconcile phase names with strategy docs
- **4.5** regression replay (~0.5h): exp45-{specs,design,brainstorm,roundtable} via dogfood, compare to exp44-post-phase7b baselines
- **4.6** close-out (~0.25h): BACKLOG, ADR-0011 addendum, plan Status finalization
...
### 4.0: audit current state (~1h)

**Goal**: produce a definitive inventory for execution sub-phases.
...
3. **`commands/{specs,design,brainstorm}.md` Phase 2 sites**: cross-reference how they currently invoke `phase-2-core.md` so roundtable.md follows the same pattern.
...
6. **Resolution hierarchy gap inventory**: confirm `default_strategy` is currently unread by any command; identify whether any other profile field has the same "documented intent, unread" status.

**Output**: `.s2s/plans/20260518-tech002-phase4-4.0-audit.md` mirroring the structure of `20260518-tech002-phase7-lite-7.0-audit.md`. Must include explicit pass-3 grep verification (lesson from Phase 7-lite 7.0 audit: pass 1 undercounted 4×).

**Exit condition**: audit file produced; per-sub-phase task lists for 4.1–4.4 finalized.

### 4.1: codify D3 triple-duplication hierarchy (~1h)

**Goal**: explicit roles for the three sources; runtime resolution order codified; SKILL.md documentation updated.
...
2. **`roundtable-strategies/SKILL.md`** v1.2.0 → v1.3.0: add a "## Strategy resolution hierarchy" section above the workflow defaults table with a Mermaid or ASCII diagram of `CLI → config.yaml → profile → error`. The existing 7-lite disclaimer banners stay.
...
**Exit condition**: D3 hierarchy is the single explanation of strategy resolution across plugin; no contradictory text remains. `grep -rn "default_strategy" skills/ commands/` returns only sites that explicitly state "plugin fallback" or quote the resolution hierarchy.

### 4.2: Option B strategy-hook parser (~1h)

**Goal**: deterministic resolution of per-strategy hook overrides from the 5 `## Strategy hooks` sections, embedded in roundtable.md.

**Actions**:
1. **Anchor fixture**: create `skills/roundtable-execution/references/strategy-hook-anchors.md` (new file, ~80 lines). Contents:
   - One row per strategy with: opening-line phrase exact match, derived override dict, target Step 2.2c field set.
   - Header note: "This file is the deterministic fixture consumed by `commands/roundtable.md` Phase 1 strategy-hook resolution. Keep in sync with `roundtable-strategies/references/{strategy}.md` `## Strategy hooks` opening lines."
2. **Parser block in roundtable.md**: insert a new "## Resolve strategy hooks" section between current lines ~199 (after strategy doc Read) and ~206 (debate handling). Block does:
   - Read `strategy-hook-anchors.md` fixture table.
   ...
   - Persist `strategy_hook_overrides` in session.yaml under `agent_state.facilitator.hook_overrides` so the facilitator agent reads it at Step 2.2c instead of inferring.
3. **Facilitator agent update**: `agents/roundtable/facilitator.md` Step 2.2c receives `agent_state.facilitator.hook_overrides` as authoritative input. If `skip: true` → no per-round overrides emitted. If override fields present → populate them per the dict instead of LLM inference.
4. **Test fixture**: add 5 unit-style assertions inside `.s2s/test-baselines/phase4-anchor-parse.md`: for each strategy doc, capture the parse output dict and freeze it. Used in §4.5 to assert no regression.

**Exit condition**: roundtable.md has deterministic hook resolution; facilitator agent receives hook policy as data; 5 fixture assertions documented.

### 4.3: expand roundtable.md to master (~1.5h)

**Goal**: replace the Phase 3 "follow the skill" stub (lines 359-437) with explicit dispatch through `phase-2-core.md`, supporting `--workflow-type {specs|design|brainstorm|roundtable}`.

**Actions**:
1. **Profile load**: in Phase 1, after `workflow_type` is determined, Read the corresponding profile YAML (`profiles/{workflow_type}.yaml`) and load `PROFILE` into the dispatch context.
2. **Phase 2 explicit dispatch**: replace lines 359-437 with a Step 2.0 → Step 2.10 dispatch block that mirrors the structure of `commands/{specs,design,brainstorm}.md`. Each step Reads the corresponding section of `phase-2-core.md` and executes. Step 2.10 is conditional on `PROFILE.has_phase_transition` (per Phase 7-lite Step 2.10 rename).
3. **Output dispatch**: Phase 3 output-type defaulting per workflow:
   - `roundtable` → `summary` (current default)
   - `specs` → `requirements`
   - `design` → `architecture`
...
6. **Line budget**: target ~600 lines total. Current 437 lines + new Phase 2 dispatch (~150 lines) + Phase 1 profile load (~20 lines) + output dispatch tweaks (~30 lines) = ~640 lines, acceptable margin.

**Exit condition**: roundtable.md can run all four workflow types end-to-end. `wc -l commands/roundtable.md` ≤ 650. specs/design/brainstorm commands are UNCHANGED (still inline; Phase 8 territory).

### 4.4: fix command drift lines 170-179 + 194-199 (~0.5h)

**Goal**: remove inline phase enumeration and keyword auto-detect drift; reconcile with strategy docs.

**Actions**:
1. **Lines 170-179 keyword table**: keep the table (it serves a UX purpose: telling user which strategy fits which keywords) but add a header note `> Keyword → strategy mapping is documented here for user discoverability. The authoritative strategy descriptions live in skills/roundtable-strategies/references/{strategy}.md.` No deletion; just disclaimer.
2. **Lines 194-199 inline enumeration**: REMOVE the `- **standard**: phases: ["discussion"]` bulleted list entirely. Replace with: `Read the strategy doc; the canonical phases live in its frontmatter or Configuration block. The parsed strategy-hook overrides from §4.2 surface phase metadata when needed.`
3. **consensus-driven phase-name fix**: this is a data fix — `consensus-driven.md` is authoritative per Phase 7-lite. The command's inline `["proposal", "discussion", "resolution"]` is wrong and is removed in step 2 above; no further fix needed.
...
### 4.5: regression replay + Option B fixture verification (~0.5h)

**Goal**: confirm no behavioral regression across the 3 workflows that have baselines; verify Option B fixture matches expected dicts.
...
2. **Anchor parse fixture**: for each of 5 strategies, run the parse block standalone (via a small probe script or inline check), assert output dict equals `phase4-anchor-parse.md` frozen values.
3. **Acceptable deltas**: debate Pro/Con assignment may shift from exp44 (LLM-emergent) to deterministic (Option B); document delta and confirm it is the intended deterministic outcome, not a structural break.
4. **Unacceptable deltas**: dump schema changes, Step 2.X numbering changes, session.yaml structural differences, missing artifacts. Any unacceptable delta blocks the PR.
5. **Smoke probe**: `/s2s:roundtable "test topic" --diagnostic --rounds 1` — confirm session.yaml.agent_state.facilitator.hook_overrides is populated (Option B working) and Step 3.0 diagnostic report renders.

**Exit condition**: 3 baselines match (structural); roundtable.md master path produces structurally-equivalent specs output; fixture assertions pass; deltas documented.

### 4.6: close-out (~0.25h)

**Actions**:
1. `.s2s/BACKLOG.md` TECH-002 block: Phase 4 marked ✅ completed; Phase 8 promoted to `in_progress` (next session); Acceptance Criteria items 2 and 4 of TECH-002 marked done.
2. `.s2s/decisions/0011-roundtable-command-unification.md` Phase 4 addendum: record Option B choice with §3.1 matrix summary, D3 hierarchy decision, drift fixes.
3. Plan Status field finalize: `draft` → `completed (PR #XX merged YYYY-MM-DD)` post-merge.
4. `MEMORY.md` `project_tech002_progress.md` updated to reflect Phase 4 done; Phase 8 + six-hats baseline as remaining items.
...
|----|------|------------|--------|------------|
| R1 | Option B fixture brittleness — opening-line phrases drift when strategy docs are edited | medium | medium | §4.2 anchor fixture is single-file; § Strategy hooks opening lines are referenced from `strategy-hook-anchors.md` header note as "must stay in sync". CI grep check (Phase 8 candidate) flags drift. |
| R2 | roundtable.md exceeds 650-line budget after Phase 2 expansion | medium | low | If overflow, move Phase 1 setup helpers to `roundtable-execution/references/phase-1-setup.md` (mirror of phase-2-core.md). Add as 4.3 fallback action. |
...
- [ ] 4.0 audit file produced; per-sub-phase task lists finalized; pass-3 grep verification recorded.
...
- [ ] Option B parser implemented: `strategy-hook-anchors.md` fixture exists; roundtable.md Phase 1 has parse block; facilitator agent consumes `agent_state.facilitator.hook_overrides`; 5 anchor assertions frozen in `phase4-anchor-parse.md`.
- [ ] roundtable.md expanded to master: Phase 2 stub replaced; `--workflow-type {specs|design|brainstorm|roundtable}` dispatches correctly; resume works for all workflow_types; `wc -l commands/roundtable.md` ≤ 650.
- [ ] specs/design/brainstorm commands UNCHANGED in this PR (Phase 8 territory).
- [ ] Lines 170-179 keyword table disclaimer-protected; lines 194-199 inline phase enumeration removed.
- [ ] consensus-driven and six-hats phase-name drift resolved (by source-of-truth deferral, no inline enumeration).
- [ ] Regression replay: 3 baselines match structurally; debate deterministic delta documented; roundtable.md master path produces structurally-equivalent specs output.
- [ ] 5 anchor parse fixture assertions pass.
- [ ] Smoke probe (`/s2s:roundtable --diagnostic --rounds 1`) populates `agent_state.facilitator.hook_overrides`; Step 3.0 renders.
- [ ] `.s2s/BACKLOG.md` TECH-002 block: Phase 4 ✅; Phase 8 `in_progress (next session)`; TECH-002 acceptance criteria 2 and 4 marked done.
- [ ] ADR-0011 Phase 4 addendum: Option B + D3 + drift fixes recorded.
...
- **CI drift check for anchor fixture**: `strategy-hook-anchors.md` opening-line phrases vs strategy-docs opening lines — small bash check. Phase 8 candidate.
- **Agent prompt sharpening**: `agents/roundtable/facilitator.md` lines 518, 579, 607 currently point to whole `{debate,consensus-driven,disney}.md`. After Phase 4, these can point to `#strategy-hooks` anchors. Low-priority polish; Phase 8 candidate.
...
3. **commands/*.md Phase 2 pattern reference** — line numbers in specs/design/brainstorm where `phase-2-core.md` is invoked, so roundtable.md mirrors the pattern.
...
6. **Resolution hierarchy gap inventory** — `default_strategy` + any other "documented intent, unread" field.
7. **Pass-3 grep verification footer** — explicit repo-wide greps run, results pasted (per Phase 7-lite 7.0 audit lesson §7.5).
...
Read("${CLAUDE_PLUGIN_ROOT}/skills/roundtable-execution/references/strategy-hook-anchors.md")
...
The facilitator agent at Step 2.2c reads `agent_state.facilitator.hook_overrides` and applies it instead of inferring per-round overrides from the strategy name.
